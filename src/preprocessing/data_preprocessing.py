...
def create_prompt_completion_with_preferences(
    row, dietary_options, meal_options
):
    dietary_prefs = (
        ", ".join([tag for tag in eval(row["tags"]) if tag in dietary_options])
        or "any"
    )
    meal_type = (
        ", ".join([tag for tag in eval(row["tags"]) if tag in meal_options])
        or "meal"
    )

    prompt = (
        f"Please write a {dietary_prefs} {meal_type} recipe that takes "
        f"approximately {row['minutes']} minutes and includes "
        f"the following ingredients: {', '.join(eval(row['ingredients']))}. "
        f"The recipe should be formatted with a clear list of ingredients "
        f" and detailed, step-by-step cooking instructions."
    )

    ingredients = "\n".join(eval(row["ingredients"]))
    steps = "\n".join(
        [f"{i+1}. {step}" for i, step in enumerate(eval(row["steps"]))]
    )
...
formatted_data_with_preferences = [
    create_prompt_completion_with_preferences(
        row, dietary_options, meal_options
    )
