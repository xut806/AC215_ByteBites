...
def get_nutrition_info(ingredients):
    ...
    for ingredient in ingredients:
        ...
        if response.status_code == 200:
            data = response.json()
            if data["foods"]:
                # we will first resort to finding values in the FNDDS database,
                # as the database includes info on raw ingredients only
                fndds_foods = [
                    food
                ...
                if fndds_foods:
                    # we will try to return the match with the most complete
                    # set of nutrients
                    ...
                    if most_complete_food.get("servingSize") is None:
                        # non of the FNDDS result has a serving size,
                        # and the serving size
                        # is a fixed value of 100g for FNDDS ingredients
                        # see:
                        # https://www.ars.usda.gov/ARSUserFiles/80400530/pdf/fndds/FNDDS_2021_2023_factsheet.pdf
                        serving_size = 100
                        serving_size_unit = "g"
                    print(
                        f"""Reminder: Raw ingredient data found
                        for {ingredient}.
                    ...
                    # Fallback:
                    # if no FNDDS data is found we will use nutrition facts
                    # from other databases
                    print(
                        f"""Reminder: No raw ingredient data found
                        for {ingredient}.
                    ...
                    if non_fndds_foods:
                        # we will try to return the match with the most
                        ...
                        # we will try to return a result
                        # with a serving size
                        # if none has a serving size,
                        # we will return the result with the
                        # most complete set of nutrients
                        ...
                        print(
                            f"""Warning: No nutrition data available
                            for {ingredient}.
                            This ingredient will not be included in
                            the nutrition facts calculation."""
                        )
                        continue
    ...
    for ingredient, info in nutrition_info_dict.items():
        ...
        if serving_size and serving_size_unit == "g":
            ...
            for nutrient_name, nutrient_info in info["nutrients"].items():
                value = (
                    nutrient_info["value"] * scale_factor
                )  # scaling serving size to 100g
...
def save_nutrition_label_as_text(
    overall_nutrition, filename="nutrition_facts.txt"
):
    """
    ...
    if "Energy" in overall_nutrition:
        ...
        formatted_output += (
            f"{'Calories':<50} {calories['value']:>8.2f} {calories['unit']}\n"
        )
        formatted_output += "-" * 100 + "\n"
    ...
    for nutrient, data in overall_nutrition.items():
        if (
            nutrient != "Energy"
        ):  # skipping calories (calories was displayed at top)
