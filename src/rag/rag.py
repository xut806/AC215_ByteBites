...
def rag_chain(embedding_model, llm, recipe_chunks, query):
    ...
    # Create a retriever object from the vector store for efficient document
    # retrieval
    ...
    template = """
    You are an assistant for generating recipes.
    Use the following pieces of retrieved context to
    create a recipe based on the user's request.
    The Generated Recipe should only include a title,
    a list of ingredients, and step-by-step cooking instructions.
...
if __name__ == "__main__":
    query = """Please write a low-sodium meal recipe that
               takes approximately 55 minutes
               and includes the following ingredients:
               tomato, beef.
               The recipe should be formatted with
               a clear list of ingredients and cooking instructions."""
