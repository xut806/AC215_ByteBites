...
# det_arch = architecture used for text localization;
# reco_arch = architecture used for text recognition;
ocr_model = ocr_predictor(
    det_arch="db_resnet50", reco_arch="crnn_vgg16_bn", pretrained=True
)

# set a lower threshold for the text detection part so that more text
# region will be reognized
...
def parse_receipt(image_path):
    """
    takes in a path to the image directory and parses images with OCR.

    Returns only the words (non-numerical) on the receipt.
    """
    ...
    for line in text.split("\n"):
        non_numeric_line = re.sub(
            r"\b\d+(\.\d+)?\b", "", line
        ).strip()  # remove numbers
        non_numeric_line = re.sub(
            r"[^a-zA-Z\s]", "", non_numeric_line
    ...
    # so that we can use majority-vote approach to determine the final entity
    # type
...
for image_path in image_paths:
    try:
        ...
        all_edible_lists.append(
            {"filename": image_path, "edible_items": edible_list}
        )
