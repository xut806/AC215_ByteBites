...
class RecipeGenerator:
    def __init__(self, bucket_name, blob_name, local_file_path, destination_folder, model_id, max_seq_length=2048, load_in_4bit=True):
    ...
    def download_and_extract_model(self):
        try:
            ...
            logger.info(f"Downloaded {self.blob_name} from {self.bucket_name} to {self.local_file_path}")
            ...
            raise HTTPException(status_code=500, detail=f"Model download or extraction failed: {e}")
    ...
    def generate_recipe(self, prompt, max_new_tokens=1024, stream_output=False):
        try:
            ...
            if stream_output and self.text_streamer:
                _ = self.model.generate(**inputs, streamer=self.text_streamer, max_new_tokens=max_new_tokens)
                return None
            else:
...
async def startup_event():
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to initialize the recipe generator.")
...
async def generate_text(request: dict):
    try:
        ...
        logger.info(f"Received generation request with prompt: {input_text} and max_length={max_length}")

        generated_text = recipe_generator.generate_recipe(prompt=input_text, max_new_tokens=max_length, stream_output=False)
